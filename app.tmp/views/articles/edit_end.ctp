...
<?php if (strcmp($this->params['url']['redirect'], 'home') == 0) { ?>
			<?php echo $form->create(null, array('url' => '/'))."\n"; ?>
			<?php echo $form->end('ホーム')."\n"; ?>
<?php } else if (strcmp($this->params['url']['redirect'], 'comments') == 0) { ?>
			<?php echo $form->create(null, array('url' => array('controller' => 'comments', 'action' => 'index', 'id' => $data['Article']['id'])))."\n"; ?>
